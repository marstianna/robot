...
def refactor_kbs(docs):
    context = "\n".join([doc.page_content for doc in docs])
    context = "将以下知识库内容:\n" + context + "\n根据语意，重新组织知识库内容，但是不能修改其原有内容，也不能新加内容。"
    return llm.llm_utils.call(context)['response']


def add_example(example: dict):
    if not validate_example():
        raise ValueError("input example is error")
    prompt_examples.update(example)
